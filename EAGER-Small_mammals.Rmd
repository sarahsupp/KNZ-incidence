...
unique(smallmammals_raw$`WATERSHED/LINE`)
...
#Okay instead of what I am doing below, I just need to separate the dataset from 1992 onwards. 
smallmammals_long1 <- smallmammals_long1 %>% subset(.$Recyear >= 1992)
smallmammals_long1 <- smallmammals_long1 %>% mutate(Recyear = factor(Recyear))
...
#   if (smallmammals_long1[i,4] == "1989" & smallmammals_long1[i,6] == "N4D"){smallmammals_long1[i,12] <- NA}
...
smallmammals_wide_random <- smallmammalsgroup1 %>% pivot_wider(names_from = "Species", values_from = "Count")

smallmammals_group_count1 <- smallmammals_wide_random %>%  
  group_by(Watershed_name, Fire_interval, Grazing) %>% 
...
smallmammals_halfspecies1 <- smallmammals_long2 %>% subset(Species %in% 
                                                             c("Peromyscus_leucopus", 
...
smallmammals_halfspecies2 <- smallmammals_long2 %>% subset(Species %in% 
                                                             c("Chaetodipus_hispidus",
                                                               "Cryptotis_parva", 
                                                               "Mus_musculus", 
...
unique(smallmammals_long2$Species)
...
smallmammals_halfspecies1 %>% 
  ggplot(aes(x = Recyear, y = Presence, group = Species, color = Species)) + 
   scale_color_manual(values = c("#F8766D", "#E38900", "#C49A00", 
                                 "#99A800", "#53B400", "#00BC56", 
                                 "#00C094")) +
  geom_point() + 
  geom_line() + 
...
smallmammals_halfspecies2 %>% 
  ggplot(aes(x = Recyear, y = Presence, group = Species, color = Species)) + 
  scale_color_manual(values = c("#00BFC4", "#00B6EB", "#06A4FF",
                                "#A58AFF", "#DF70F8", "#FB61D7", 
                                "#FF66A8")) +
  geom_point() + 
  geom_line() + 
  facet_grid(Species~Grazing~Fire_interval, switch = "y") +
...
smallmammals_years_wide2 <-smallmammals_years_wide %>% mutate_all(~replace(., is.na(.), 0))

#Summing the incidence of all years into a new column
smallmammals_years_wide2 <- smallmammals_years_wide2 %>% mutate(Total_Persistence = 
                                                                rowSums(smallmammals_years_wide2[, 5:26]))
view(smallmammals_years_wide2)

#Now I'm am going to make a for loop to calculate incidence by taking the total incidence and dividing it by the number of total years. The ungrazed and bison ones will have a different number of years.

smallmammals_years_wide2 <- smallmammals_years_wide2 %>% mutate(Persistence = (.$Total_Persistence/22))

#Just getting a table to show the species persistence by fire-interval, grazing, and years total
Persistence_table <- smallmammals_years_wide2 %>% group_by(Fire_interval, Grazing, Species) %>% 
  dplyr::reframe(Persistence = sum(Persistence))

sort(Persistence_table$Persistence, decreasing = TRUE)
...
#Now I'm basically going to do everything over again, but with abundance instead?
...
smallmammals_wide3 <- smallmammals_wide %>% mutate(across(where(is.character), as.numeric))
#Standardizing by presence/absence
decostand_tot_smallmammals <- decostand(smallmammals_wide3[, -c(1:4)], "total", na.rm = TRUE )

#Checking the stress of this presence/absence data
dimcheckMDS(decostand_tot_smallmammals, distance = "bray", k=5)
#making an mds 
idk_mds2 <- metaMDS(decostand_tot_smallmammals, distance = "bray", k = 2, trymax = 30)




#Adding the presence/absence dataset back with the enviornment dataset
smallmammals_wide4 <- decostand_tot_smallmammals %>% add_column(smallmammals_env, .before = "Blarina_hylophaga")

#Pivoting the presence/absence back to a longer format
smallmammals_long3 <- smallmammals_wide4 %>% pivot_longer(cols = 5:18, 
                                                          names_to = "Species", 
                                                          values_to = "Abundance")
smallmammals_long3 <- smallmammals_long3 %>% mutate(Species = factor(Species))




smallmammals_halfspecies3 <- smallmammals_long3 %>% subset(Species %in% 
                                                             c("Peromyscus_leucopus", 
                                                               "Peromyscus_maniculatus", 
                                                               "Blarina_hylophaga", 
                                                               "Microtus_ochrogaster",
...
smallmammals_halfspecies4 <- smallmammals_long3 %>% subset(Species %in% 
                                                             c("Chaetodipus_hispidus",
                                                               "Cryptotis_parva", 
                                                               "Mus_musculus", 
                                                               "Reithrodontomys_megalotis", 
...
smallmammals_halfspecies3 %>% 
  ggplot(aes(x = Recyear, y = Abundance, group = Species, color = Species)) + 
   scale_color_manual(values = c("#F8766D", "#E38900", "#C49A00", 
                                 "#99A800", "#53B400", "#00BC56", 
                                 "#00C094")) +
  geom_point() + 
...
for(i in 1:nrow(smallmammals_cont)){
   if (smallmammals_cont[i,1] == "1992"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1993"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1994"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1995"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1996"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1997"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1998"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "1999"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "2000"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "2001"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "2002"){smallmammals_cont[i,8] <- "Early"}
   if (smallmammals_cont[i,1] == "2003"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2004"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2005"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2006"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2007"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2008"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2009"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2010"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2011"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2012"){smallmammals_cont[i,8] <- "Late"}
   if (smallmammals_cont[i,1] == "2013"){smallmammals_cont[i,8] <- "Late"}
}
