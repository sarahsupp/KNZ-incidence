...
smammals_years_wide <- smammals_years_wide %>% 
  mutate(Persistence = (.$Total_Presence/22))
...
#Filling presence contingency column based on presence/absence
 # for(j in 1:nrow(smammals_cont)){
 #   if (smammals_cont[j,10] == 1){smammals_cont[j,11] <- "Present"}
 #   if (smammals_cont[j,10] == 0){smammals_cont[j,11] <- "Absent"}
 # } 

#Filling early/late contingency based on year
for(i in 1:nrow(smammals_cont)){
   if (smammals_cont[i,9] < 2003){smammals_cont[i,12] <- "Early"}
   if (smammals_cont[i,9] >= 2003){smammals_cont[i,12] <- "Late"}
}
