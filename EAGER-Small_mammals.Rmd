...
summary_tbl2_wide <- summary_tbl2 %>% 
  pivot_wider(names_from = Species_Watershed, values_from = n)

contabs <- xtabs(n ~ Presence_cont + Early_late + Species_Watershed, summary_tbl2)
apply(contabs, 3, chisq.test)

chisq_p.values<- tibble(do.call(rbind,lapply(chisqtests,function(v){v$p.value})))
