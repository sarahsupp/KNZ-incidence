...
```{r}
unique(grasshoppers_wide$Watershed_name)

#This section is to check the similarity between two sites of the same fire/grazing treatment
grasshoppers_N1 <- grasshoppers_cae2_short %>% subset(Watershed_name %in% c("N1A", "N1B"))
grasshoppers_N1 <- grasshoppers_N1 %>% group_by(Watershed_name, Fire_interval, Grazing, Species) %>% 
  reframe(Total = sum(Total))


grasshoppers_N1wide <- grasshoppers_N1 %>% 
  pivot_wider(names_from = "Species", values_from = "Total", 
              values_fn = function(x) paste(sum(x)))

grasshoppers_N1wide_short <- grasshoppers_N1wide[, -c(1:3)]
grasshoppers_N1wide_short <- grasshoppers_N1wide_short %>% mutate_if(is.character, as.numeric)
grasshoppers_N1wide_short[is.na(grasshoppers_N1wide_short)] <- 0

sum(apply(grasshoppers_N1wide_short, 2, function(x) abs(max(x)-min(x)))) / sum(rowSums(grasshoppers_N1wide_short))

decostand_bray_grasshoppersN1 <- decostand(grasshoppers_N1wide_short, "total", na.rm = FALSE)
all_mds_bray <- metaMDS(decostand_bray_grasshoppersN1, distance = "bray", k=1, trymax = 40)





grasshoppers_N4 <- grasshoppers_cae2_short %>% subset(Watershed_name %in% c("N4A", "N4D"))
grasshoppers_N4 <- grasshoppers_N4 %>% group_by(Watershed_name, Fire_interval, Grazing, Species) %>% 
  reframe(Total = sum(Total))

grasshoppers_N4wide <- grasshoppers_N4 %>% 
  pivot_wider(names_from = "Species", values_from = "Total", 
              values_fn = function(x) paste(sum(x)))

grasshoppers_N4wide_short <- grasshoppers_N4wide[, -c(1:3)]
grasshoppers_N4wide_short <- grasshoppers_N4wide_short %>% mutate_if(is.character, as.numeric)
grasshoppers_N4wide_short[is.na(grasshoppers_N4wide_short)] <- 0

sum(apply(grasshoppers_N4wide_short, 2, function(x) abs(max(x)-min(x)))) / sum(rowSums(grasshoppers_N4wide_short))
...
grasshoppers_N20 <- grasshoppers_cae2_short %>% subset(Watershed_name %in% c("N20A", "N20B"))
grasshoppers_N20 <- grasshoppers_N20 %>% group_by(Watershed_name, Fire_interval, Grazing, Species) %>% 
  reframe(Total = sum(Total))

grasshoppers_N20wide <- grasshoppers_N20 %>% 
  pivot_wider(names_from = "Species", values_from = "Total", 
              values_fn = function(x) paste(sum(x)))

grasshoppers_N20wide_short <- grasshoppers_N20wide[, -c(1:3)]
grasshoppers_N20wide_short <- grasshoppers_N20wide_short %>% mutate_if(is.character, as.numeric)
grasshoppers_N20wide_short[is.na(grasshoppers_N20wide_short)] <- 0

sum(apply(grasshoppers_N20wide_short, 2, function(x) abs(max(x)-min(x)))) / sum(rowSums(grasshoppers_N20wide_short))
...
grasshoppers7 <- grasshoppers_cae2 %>% group_by(Watershed_name, Fire_interval, Grazing, Species) %>% 
  reframe(Total = sum(Total))
...
diss_matrix <- vegdist(grasshoppers7_wide1, method = "bray")
```
