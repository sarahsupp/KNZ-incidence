...
##Creating the needed data frame by subsetting the year 1992, then grouping the smammals data frame by watershed and species and just pasting the species counts, then pivoting so that each watershed is a row and each species is a column
smammals_jacc_diss_year1 <- smammals %>% 
  subset(Recyear == 1992) %>%
...
smammals_jacc_diss_year1 <- smammals_jacc_diss_year1[, -c(1)] %>% 
  decostand(method = "pa") %>% 
...
smammals_jacc_diss_year1 <- as.matrix(smammals_jacc_diss_year1)
dimnames(smammals_jacc_diss_year1) <- list(c("1D", "20B", "4B", 
                                             "N1B", "N20B", "N4D"), 
                                           c("1D", "20B", "4B", 
                                             "N1B", "N20B", "N4D"))
...
smammals_diss <- as.data.frame.table(as.matrix(smammals_jacc_diss_year1))
...
smammals_diss <- smammals_diss %>% r
ename("JaccDiss_year22" = "Freq")
