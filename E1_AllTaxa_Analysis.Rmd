...
Then we can use this function to get the abundance trends.
...
We also need functions to extract information from linear models. These will be used later for analyzing species richness across each watershed.
```{r get model results}
...
### Small-mammals: Dissimilarity Between Watersheds and between Years
...
smammals_jacc <-  smammals_count_allyears[, -c(1)] %>% 
  decostand(method = "pa") %>% 
...
smammals_jacc_allyears <- as.data.frame.table(as.matrix(smammals_jacc)) %>%
...
#FIXME: MAKE THIS DATAFRAME LESS REDUNDANT (DELETE LOWER TRIANGE?)
## Transform the Jaccard's dissimilarity matrix into a data frame
# Rename the automatically created column names
# Filter Year1 to always be 1992 so all comparisons are relative to the baseline year
# Keep only comparisons within watershed
# ensure years are numeric
smammals_jacc_watersheds <- as.data.frame.table(as.matrix(smammals_jacc)) %>%
  separate(Var1, c("Watershed1", "Recyear"), "_") %>%
  separate(Var2, c("Watershed2", "Recyear2"), "_") %>%
  rename("Year1" = "Recyear",
         "Year2" = "Recyear2" ,
...
##Standardize for the percent abundance of each species based on total abundance across each watershed
#   Conduct a Bray-Curtis dissimilarity test on the output
smammals_bc <- smammals_count_allyears[, -c(1)] %>% 
...
## Transform the Bray-Curtis dissimilarity matrix into a data frame
# Rename the automatically created column names
# Filter Year1 to always be 1992 so all comparisons are relative to the baseline year
# Keep only comparisons within watershed
# ensure years are numeric
smammals_bc_allyears <- as.data.frame.table(as.matrix(smammals_bc)) %>%
  separate(Var1, c("Watershed1", "Recyear"), "_") %>%
  separate(Var2, c("Watershed2", "Recyear2"), "_") %>%
  rename("Year1" = "Recyear",
         "Year2" = "Recyear2" ,
...
## Transform the Jaccard's dissimilarity matrix into a data frame
# Rename the automatically created column names
# Filter Year1==Year2 so watersheds are compared within the same year
# Keep only comparisons within watershed
# ensure years are numeric
smammals_bc_watersheds <- as.data.frame.table(as.matrix(smammals_bc)) %>%
  separate(Var1, c("Watershed1", "Recyear"), "_") %>%
  separate(Var2, c("Watershed2", "Recyear2"), "_") %>%
  rename("Year1" = "Recyear",
         "Year2" = "Recyear2" ,
...
smammals_diss_year1toN <- merge(smammals_jacc_allyears, smammals_bc_allyears)

smammals_diss_watersheds <- merge(smammals_jacc_watersheds, smammals_bc_watersheds)
...
Visualize the Jaccards results
```{r}
smammals_jacc_plot <- 
  ggplot(data=smammals_diss_year1toN, aes(x=Year2, y=Jacc_diss)) + 
  geom_point() +
...
Visualize the Bray-Curtis results
```{r}
smammals_bray_plot <- 
  ggplot(data=smammals_diss_year1toN, aes(x=Year2, y=Bray_diss)) + 
  geom_point() +
  geom_smooth(method="lm") +
  ggpubr::stat_cor() +
  ylim(0,1) +
...
```{r}
smammals_sprich %>% ggplot(aes(x = Recyear, y = Richness)) + 
  geom_point() + 
...
# Take the table already created for the species counts, then 
#   pivot the data into a long format,
#   where 'Recyear' column is a numeric column, and 
#   the 'Watershed_name', 'Species', and 'Common_name' columns into a factor
smammals_spabund_long <- smammals_years_wide %>%
  pivot_longer(cols = "1992":"2013",
               names_to = "Recyear",
               values_to = "Count") %>%
  mutate(Recyear = as.numeric(Recyear),
...
#FIXME: add this plot to E2 file instead?
Visualize the species with significantly increasing or decreasing abundance trends
```{r}
# this is a mess to visualize, let's try only showing the significant ones
smammals_spabund_sig <- smammals_spabund_long %>% 
  filter()
