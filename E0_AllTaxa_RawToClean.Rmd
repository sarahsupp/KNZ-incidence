...
author: "Maya Parker-Smith and Sarah R. Supp"
date: "2023-11-13"
output:
  html_document:
    smart: no
...
**Column name conventions for E1:**
All four cleaned datasets are written with the following standardized columns so that E1 can treat taxa consistently:
- `Watershed_name` — watershed identifier (short code, e.g. "1D", "N20B")
- `Recyear` — recording year (numeric)
- `Species` — species identifier. For smammals/grasshoppers this is the binomial name; for birds this is the AOU code (the original `AOU_code` column is kept alongside it); for plants this is `Genus_spp` (the original `Genus_spp` column is kept alongside it).
- `Count` — the abundance value per row. For smammals, birds, and grasshoppers this is an individual count; for plants this is the cover class value (ordered 1–7). The original column names (`Total` for grasshoppers, `Cover` for plants) are kept alongside `Count` for reference.
...
#Separate the watershed/line column into two separate columns and create a new dataset
smammals_all <- separate(raw_smammals, `WATERSHED/LINE`, into = c("WATERSHED", "LINE"), sep = "-")

#Rename columns just so that they're not all capitalized
...
#Shorten the names of the watersheds
...
#Pivot the table so that each species per year and watershed is a different row
smammals <- pivot_longer(smammals_all, cols = 8:21, 
                              names_to = "Species", 
                              values_to = "Count")

#Rename the species into their full names
...
# Remove the 4F watershed to make sampling between all fire/grazing treatments equal (the 4 year fire interval with no grazing combination of disturbances has two watersheds represented, while all other combinations have just one).
smammals <- smammals %>% subset(Watershed_name != "4F")

#Create table with small mammal common names, to join to the main dataset and make referencing easier in plots
smammals_commonnames <- tibble(Species = c("Blarina_hylophaga", 
                                           ...
                                           "Synaptomys_cooperi", 
                                           "Zapus_hudsonius"), 
                               Common_name = c("Elliot's short-tailed shrew", 
                                               "Hispid pocket mouse", 
...
#Join the common names data table to the main dataset
smammals <- left_join(smammals, smammals_commonnames, by = "Species")

# Smammals already uses Species and Count as column names — no renaming needed for E1 standardization.

#Write the cleaned smammals dataset to the 'Cleaned_data' folder in the project directory
...
#Read in the raw data; this dataset is the grasshopper species counts from each set of sweeps
raw_grasshoppers <- read_csv("Datasets/Raw_data/Grasshoppers/CGR022.csv", show_col_types = FALSE) 

#Dataset Maya created that records the families and suborders for each grasshopper genus in the dataset; I am going to only look at grasshoppers from the suborder Caelifera because those from the suborder Ensifera (family/genus: Oecanthinae, Gryllidae, and Tettigoniidae) were not counted until 2013.
grasshopper_families <- read_excel("Datasets/Raw_data/Grasshoppers/Grasshopper_families.xlsx") 

#Change the classifications of the some of the variables for later analysis and create a new dataset
grasshoppers_all <- raw_grasshoppers %>% mutate(WATERSHED = factor(WATERSHED),
                                                SPECIES = factor(SPECIES),
                                                SPCODE = factor(SPCODE),
                                                RECYEAR = as.numeric(RECYEAR))

#Check the number of unique names for the species to check for misspellings
...
#Recode some of the species to make it consistent
...
#Check the watershed names to look for misspellings 
...
#Rename watersheds to make it consistent
...
##Consolidate watersheds based on the metadata
grasshoppers_all$WATERSHED <- dplyr::recode(grasshoppers_all$WATERSHED,
                                   "B" = "20B",
                                   "NB" = "N20B",
                                   "2D" = "SpB",
...
###Some species names aren't correctly matched with their spp code, so I will fix that
...
#Rename the columns so they're not all capitalized
...
#Select only the watersheds I will use for further analysis
grasshoppers <- grasshoppers %>%
  subset(Watershed_name %in% c("1D", "20B", "4B", 
                               "4F", "N1B", "N4A", 
                               "N4D", "N20B")) 

#Separate the genus and spp and creating new columns. This is to add in the family and suborder info and also to easily remove entries that are not identified down to species.
grasshoppers <- grasshoppers %>% 
  separate(Species, c("Genus", "Spp"), sep = " ", remove = FALSE)


## Add in dataset that has the family and suborder info
##  Then only select those from the suborder Caelifera since suborder Ensifera are katydids and crickets and most were not counted until 2013
grasshoppers <- left_join(grasshoppers, grasshopper_families, by = "Genus")
grasshoppers <- grasshoppers %>% subset(Suborder == "Caelifera")

##Remove unknowns from the dataset
grasshoppers <- grasshoppers %>% subset(Genus != "Unknown")

##Remove entries that aren't identified to species 
grasshoppers <- grasshoppers %>% subset(Spp != "spp.")

# Standardize for E1: add Count as a copy of Total so all taxa share the same abundance column name.
# The original Total column is kept for reference.
grasshoppers <- grasshoppers %>% mutate(Count = Total)

#Write the cleaned grasshoppers dataset to the 'Cleaned_data' folder in the project directory
...
#Change classifications and names of columns
...
#Select only the watersheds I want for further analysis
...
#Check the lengths of the species (the 'specname' variable has a lot more entries, but they are duplicates with naming variations when looking at the matching common names. So I'll use the 'Species' column [AOU code] instead.)
length(unique(birds$Specname))
length(unique(birds$Species))

#Remove observations that are unspecified
birds <- subset(birds, !(Species %in% c("NONE", "VOID", "UNSP", "UNEM", "RSTO")))

#Rename this one lowercase entry to be consistent with other uppercase entries
birds$Species <- dplyr::recode(birds$Species, "eawp" = "EAWP")

# Note for E1: Species now holds the AOU code (the reliable species identifier).
# Sci_name holds the original scientific name (SPECNAME) for reference.
# Count is already the individual observation count.

#Write the cleaned birds dataset to the 'Cleaned_data' folder in the project directory
...
#Combine genus and species into a new column, but keep Genus and Spp columns
plants_all <- unite(plants_all, "Genus_spp", AB_genus:AB_species, sep = "_", remove = FALSE)

#Rename the watersheds to make everything consistent
...
#Remove data pre-1992 since bison weren't introduced until then
plants <- plants_all %>% subset(.$Recyear >= 1992)

#Select only the watersheds and I will use for further analysis
plants <- plants %>%
  subset(Watershed_name %in% c("1D", "4A", "4B", 
                               "20B", "N1B", "N4A", 
                               "N4D", "N20B")) 

#Remove the observations that aren't identified down to species
plants <- subset(plants,!(Genus_spp %in% c("annual_forb", 
                                           "carex_spp.", 
                                           "cyperu_spp.", 
                                           "euphor_spp.", 
                                           "symphy_spp.")))

#There is one capitalized entry in Genus_spp; change it to match everything else
plants$Genus_spp<- dplyr::recode(plants$Genus_spp, "Lithos_occid" = "lithos_occid")

# Standardize for E1: plants uses Genus_spp as the species identifier and Cover (ordered
# class 1-7) as the abundance measure, unlike the count-based taxa. Add Species as a copy
# of Genus_spp, and Count as a copy of Cover, so all taxa share the same column names in E1.
# The original Genus_spp and Cover columns are both kept for reference.
plants <- plants %>% mutate(Species = Genus_spp,
                            Count = Cover)
