...
birds_1 <- birds_raw %>% dplyr::select(RECYEAR, 
                                       RECMONTH, 
                                       RECDAY, 
                                       SEASON, 
                                       TRANSNUM,
...
birds_1 <- birds_1 %>% mutate(WATERSHED = case_when(
    WATERSHED == "N04D" ~ "N4D",
    WATERSHED == "N04B" ~ "N4B",
    WATERSHED == "004B" ~ "4B",
    WATERSHED == "004A" ~ "4A",
...
birds_1 <- birds_1 %>% rename("Watershed_name" = "WATERSHED",
                              "Recyear" = "RECYEAR",
                              "Recmonth" = "RECMONTH",
                              "Recday" = "RECDAY",
                              "Transect" = "TRANSNUM",
...
birds_2 <- left_join(birds_1, watershed_info, by = "Watershed_name")
...
birds_3 <- birds_2 %>% subset(.$Recyear >= 1992)
birds_3 <- birds_3 %>% mutate(Recyear = factor(Recyear))
...
birds_3_group <- birds_3 %>% 
  group_by(Recyear, Transect, Recmonth, Season, Watershed_name, Species, Fire_interval, Grazing) %>%
...
birds_3_group_wide <- birds_3_group %>% pivot_wider(names_from = Species, values_from = Count)

birds_group_count1 <- birds_3_group_wide %>% group_by(Watershed_name) %>% 
  summarise(total_count=n(),.groups = 'drop')


birds_group_count2 <- birds_3_group_wide %>% group_by(Recyear, Watershed_name) %>% 
  summarise(total_count=n(),.groups = 'drop')

birds_group_count2 %>% 
  ggplot(aes(x = RECYEAR, y = total_count, color = WATERSHED)) + 
  geom_jitter(width = 0.2, height = 0.2)


birds_group_count3 <- birds_3_group_wide %>% group_by(Season, Transect) %>% 
  summarise(total_count=n(),.groups = 'drop')


birds_group_count3 <- birds_3_group_wide %>% group_by(Recyear, Transect, Watershed_name) %>% 
  summarise(total_count=n(),.groups = 'drop')

birds_group_count4 <- birds_3_group_wide %>% group_by(Season, Recmonth, Watershed_name) %>% 
  summarise(total_count=n(),.groups = 'drop')

birds_group_count5 <- birds_3_group_wide %>% group_by(Transect, Watershed_name, Fire_interval, Grazing) %>% 
  summarise(total_count=n(),.groups = 'drop')

birds_2_group <- birds_3_group %>% mutate(Recyear = factor(Recyear))
birds_2_group %>% subset(.$Watershed_name == "N20B") %>% ggplot(aes(x = Recyear, y = Count)) + 
  geom_col() 
...
##Oh lord... there's capitalizations in some but not others and there are some unknowns. I'll have to alphabetize it and go through to see if there are multiple spellings for the same species
...
grasshoppers_1 <- grasshoppers %>% mutate(WATERSHED = case_when( 
                                 WATERSHED == "001d" ~ "1D",
                                 WATERSHED == "004b" ~ "4B",
                                 WATERSHED == "000b" ~ "B",
                                 WATERSHED == "n04a" ~ "N4A",
...
grasshoppers_2 <- grasshoppers_1

grasshoppers_2 <- grasshoppers_1 %>% mutate(WATERSHED = case_when(
                                  WATERSHED == "B" ~ "20B",
                                  WATERSHED == "NB" ~ "N20B",
                                  WATERSHED == "2D" ~ "SpB",
                                  WATERSHED == "4D" ~ "SpB",
...
grasshoppers_2 <- left_join(grasshoppers, watershed_info, by = "Watershed_name")
view(grasshoppers_2)
...
grasshoppers_2_group <- grasshoppers_2 %>% group_by(Recyear, Recmonth, Watershed_name, Species) %>% 
...
grasshoppers_group_count1 <- grasshoppers_2_group_wide %>% group_by(Watershed_name, Recmonth) %>% 
  summarise(total_count=n(),.groups = 'drop')
...
smallmammals <- smallmammals %>% mutate(WATERSHED = case_when(
                                       WATERSHED == "004F" ~ "4F",
                                       WATERSHED == "004B" ~ "4B",
                                       WATERSHED == "001D" ~ "1D",
                                       WATERSHED == "020B" ~ "20B",
...
smallmammals <- smallmammals %>% rename("Peromyscus_maniculatus" = "Pm", 
                                        "Reithrodontomys_megalotis" ="Rmeg",
                                        "Sigmodon_hispidus" = "Sh",
                                        "Blarina_hylophaga" = "Bh",
                                        "Reithrodontomys_montanus" = "Rmon", 
                                        "Spermophilus_tridecemlineatus" = "St",
                                        "Microtus_ochrogaster" = "Mo",
                                        "Peromyscus_leucopus" = "Pl",
                                        "Chaetodipus_hispidus" = "Ch", 
                                        "Mus_musculus" = "Mm",
                                        "Neotoma_floridana" = "Nf", 
                                        "Synaptomys_cooperi" = "Sc", 
                                        "Zapus_hudsonius" = "Zh", 
                                        "Cryptotis_parva" = "Cp")
...
smallmammals <- left_join(smallmammals, watershed_info, by = "Watershed_name")


smallmammals_1 <- smallmammals %>% subset(.$Recyear >= 1992)
smallmammals_1 <- smallmammals_1 %>% mutate(Recyear = factor(Recyear))

#Pivoting the table so that each species is a column and each species per year is a different row
smallmammals_2 <- pivot_longer(smallmammals_1, 
...
smallmammals_group_count1 <- smallmammals_1 %>%  group_by(Watershed_name) %>% 
  summarise(total_count=n(),.groups = 'drop')
